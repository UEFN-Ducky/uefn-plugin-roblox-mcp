#!/usr/bin/env python3
"""Zip + publish roblox-mcp to the UEFN Ducky Store via uds_release.
...
Usage:
  ...
  py scripts/release.py --publish --changelog "v1.0.0: ROBLOX MCP Studio + Open Cloud"
"""
...
def publish(zip_path: Path, *, category: str, changelog: str) -> None:
    ...
    if "duckyos.org" in base and "uefnducky.org" not in base:
        raise SystemExit(
            f"Refusing base URL {base!r} — publish ROBLOX MCP to https://uefnducky.org only"
        )
    ...
    label = str(manifest.get("label") or "ROBLOX MCP")
    pid = str(manifest.get("id") or "roblox-mcp")
    ...
    release_args: dict = {
        ...
        "name": label,
        "slug": pid,
        "categories": ["plugins", "mcps"],
        "tags": ["roblox", "mcp", "studio", "luau", "open-cloud"],
    }
    if file_id:
